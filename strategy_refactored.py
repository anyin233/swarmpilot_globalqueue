...
class ShortestQueueStrategy(BaseStrategy):
    ...
    This strategy prioritizes instances with the lowest expected_ms value,
    which typically means the instance with the least workload.
    """
    ...
    def _select_from_candidates(
        ...
        """
        Select the instance with the shortest expected completion time
        ...
        Returns:
            Instance with minimum expected_ms
        """
        # Sort by expected_ms and select the one with shortest time
        selected = min(candidates, key=lambda x: x[1].expected_ms)

        logger.debug(
            f"ShortestQueueStrategy selected: instance {selected[0].uuid} "
            f"with expected_ms={selected[1].expected_ms}"
        )
